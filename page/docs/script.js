document.addEventListener("DOMContentLoaded", async () => {
  ...
  const populatePageContent = () => {
    ...
    const creator = settings.apiSettings?.creator || "4Bq0r1 Api'S"

    setPageContent(DOM.pageTitle, settings.name, "4Bq0r1 Api'S")
    setPageContent(DOM.wm, `© ${currentYear} Migi Api'S Corp. All rights reversed.`)
    setPageContent(DOM.appName, settings.name, "4Bq0r1 Api'S")
    ...
    if (DOM.dynamicImage) {
      if (settings.bannerImage) {
        ...
        DOM.dynamicImage.src = "/src/images/banner.png"
    ...
    if (DOM.apiLinksContainer) {
      ...
      const defaultLinks = [{ url: "https://github.com/abqoryme", name: "abqoryme", icon: "fab fa-github" }]
